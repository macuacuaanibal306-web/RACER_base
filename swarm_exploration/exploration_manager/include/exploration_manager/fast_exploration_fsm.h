...
private:
  ...
  void updateStopReason(const string& reason, const string& detail = "");
  void clearStopReason(const string& reason = "");
  void logStopReason(const string& prefix = "STOP_DIAG") const;
  void logHeartbeat(const string& prefix = "FSM_HEARTBEAT") const;
  void visualize(int content);
  void clearVisMarker();
  int getId() const;
  bool isInCommunicationRange(const Eigen::Vector3d& other_pos) const;
  Eigen::Vector3d getCurrentBroadcastPos() const;
  int getConnectedDroneCount(std::vector<int>* connected_ids = nullptr) const;
  void publishCommunicationStatus();
  void publishIntentVisualization();
  void updateDroneIntentPrediction(DroneState& state);
  bool getDroneIntentGoal(const DroneState& state, Eigen::Vector3d& goal) const;
  void communicationVisTimerCallback(const ros::TimerEvent& e);
  ...
  ros::Timer drone_state_timer_, opt_timer_, swarm_traj_timer_, comm_vis_timer_;
};
