...
void SDFMap::initMap(ros::NodeHandle& nh) {
  ...
  nh.param("exploration/drone_id", diag_drone_id_, -1);
  nh.param("map_debug/enable_boundary_diag", boundary_diag_enable_, true);
  nh.param("map_debug/diag_period", boundary_diag_period_, 1.0);
  ...
  if (boundary_diag_enable_) {
    ROS_WARN_STREAM("[map_diag][drone " << diag_drone_id_
                    << "][sdf_init] map_origin=" << fmtVec3d(mp_->map_origin_)
                    << ", map_size=" << fmtVec3d(mp_->map_size_)
                    << ", map_min_boundary=" << fmtVec3d(mp_->map_min_boundary_)
                    << ", map_max_boundary=" << fmtVec3d(mp_->map_max_boundary_)
                    << ", box_min=" << fmtVec3d(mp_->box_mind_)
                    << ", box_max=" << fmtVec3d(mp_->box_maxd_)
                    << ", box_min_idx=" << fmtVec3i(mp_->box_min_)
                    << ", box_max_idx=" << fmtVec3i(mp_->box_max_)
                    << ", resolution=" << mp_->resolution_
                    << ", optimistic=" << mp_->optimistic_
...
void SDFMap::inputPointCloud(const pcl::PointCloud<pcl::PointXYZ>& points, const int& point_num,
  ...
  if (boundary_diag_enable_) {
    ROS_WARN_STREAM_THROTTLE(boundary_diag_period_,
        "[map_diag][drone " << diag_drone_id_ << "][input_cloud] camera=" << fmtVec3d(camera_pos)
                               << ", point_num=" << point_num
                               << ", raw_update_min=" << fmtVec3d(update_min)
                               << ", raw_update_max=" << fmtVec3d(update_max)
                               << ", accum_update_min=" << fmtVec3d(md_->update_min_)
                               << ", accum_update_max=" << fmtVec3d(md_->update_max_)
                               << ", all_min=" << fmtVec3d(md_->all_min_)
                               << ", all_max=" << fmtVec3d(md_->all_max_)
                               << ", local_bound_min=" << fmtVec3i(md_->local_bound_min_)
                               << ", local_bound_max=" << fmtVec3i(md_->local_bound_max_)
                               << ", box_min=" << fmtVec3d(mp_->box_mind_)
                               << ", box_max=" << fmtVec3d(mp_->box_maxd_)
                               << ", map_min_boundary=" << fmtVec3d(mp_->map_min_boundary_)
                               << ", map_max_boundary=" << fmtVec3d(mp_->map_max_boundary_));
