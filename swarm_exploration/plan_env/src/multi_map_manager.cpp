...
void MultiMapManager::init() {
  ...
  node_.param("communication/enable_constraint", enable_comm_constraint_, false);
  node_.param("communication/range", comm_range_, 1e9);
  ...
  if (boundary_diag_enable_) {
    ROS_WARN_STREAM("[map_diag][drone " << drone_id_ << "][multi_map_init] map_num=" << map_num_
                    << ", vis_drone_id=" << vis_drone_id_
                    << ", chunk_size=" << chunk_size_
                    << ", stamp_topic_send=" << stamp_pub_.getTopic()
                    << ", chunk_topic_send=" << chunk_pub_.getTopic());
  }
...
bool MultiMapManager::isInCommunicationRange(const Eigen::Vector3d& other_pos) const {
  if (!enable_comm_constraint_) return true;
  return (drone_pos_ - other_pos).norm() <= comm_range_ + 1e-6;
}
...
void MultiMapManager::chunkTimerCallback(const ros::TimerEvent& e) {
  ...
  for (int i = 0; i < chunk_buffer_.size(); ++i) {
    ...
    if (boundary_diag_enable_ && chunk_boxes_[i].valid_) {
      ROS_WARN_STREAM_THROTTLE(boundary_diag_period_,
          "[map_diag][drone " << drone_id_ << "][chunk_timer] source_drone=" << (i + 1)
                                 << ", chunk_box_min=" << fmtVec3d(chunk_boxes_[i].min_)
                                 << ", chunk_box_max=" << fmtVec3d(chunk_boxes_[i].max_)
