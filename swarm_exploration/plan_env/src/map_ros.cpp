...
namespace {
std::string fmtVec3d(const Eigen::Vector3d& v) {
  std::ostringstream oss;
  oss << std::fixed << std::setprecision(2) << "[" << v[0] << "," << v[1] << "," << v[2]
      << "]";
  return oss.str();
}

std::string fmtVec3i(const Eigen::Vector3i& v) {
  std::ostringstream oss;
  oss << "[" << v[0] << "," << v[1] << "," << v[2] << "]";
  return oss.str();
}
...
void appendMapDiagLine(int drone_id, const std::string& tag, const std::string& line) {
...
void MapROS::init() {
  ...
  node_.param("map_debug/enable_boundary_diag", boundary_diag_enable_, true);
  node_.param("map_debug/diag_period", boundary_diag_period_, 1.0);
  ...
  if (boundary_diag_enable_) {
    ROS_WARN_STREAM("[map_diag][drone " << drone_id_
                    << "][map_ros_init] map_all_topic=" << map_all_pub_.getTopic()
                    << ", map_local_topic=" << map_local_pub_.getTopic()
                    << ", local_inflate_topic=" << map_local_inflate_pub_.getTopic()
                    << ", unknown_topic=" << unknown_pub_.getTopic()
                    << ", esdf_topic=" << esdf_pub_.getTopic()
                    << ", update_range_topic=" << update_range_pub_.getTopic()
                    << ", depth_topic=" << depth_pub_.getTopic()
                    << ", show_all_map=" << show_all_map_
                    << ", frame_id=" << frame_id_);
  }
}
...
void MapROS::publishMapAll() {
  ...
  if (boundary_diag_enable_) {
    ROS_WARN_STREAM_THROTTLE(boundary_diag_period_,
        "[map_diag][drone " << drone_id_ << "][publish_all] topic=" << map_all_pub_.getTopic()
                               << ", all_min=" << fmtVec3d(map_->md_->all_min_)
                               << ", all_max=" << fmtVec3d(map_->md_->all_max_)
                               << ", min_idx=" << fmtVec3i(min_idx)
                               << ", max_idx=" << fmtVec3i(max_idx)
                               << ", map_origin=" << fmtVec3d(map_->mp_->map_origin_)
                               << ", map_size=" << fmtVec3d(map_->mp_->map_size_)
                               << ", box_min=" << fmtVec3d(map_->mp_->box_mind_)
                               << ", box_max=" << fmtVec3d(map_->mp_->box_maxd_));
  }
  // for (int k = 0; k < 2; ++k) {
...
void MapROS::publishMapLocal() {
  ...
  if (boundary_diag_enable_) {
    ROS_WARN_STREAM_THROTTLE(boundary_diag_period_,
        "[map_diag][drone " << drone_id_ << "][publish_esdf] topic=" << esdf_pub_.getTopic()
                               << ", local_bound_min=" << fmtVec3i(map_->md_->local_bound_min_)
                               << ", local_bound_max=" << fmtVec3i(map_->md_->local_bound_max_)
                               << ", min_cut=" << fmtVec3i(min_cut)
                               << ", max_cut=" << fmtVec3i(max_cut)
                               << ", map_origin=" << fmtVec3d(map_->mp_->map_origin_)
                               << ", map_size=" << fmtVec3d(map_->mp_->map_size_)
                               << ", box_min=" << fmtVec3d(map_->mp_->box_mind_)
                               << ", box_max=" << fmtVec3d(map_->mp_->box_maxd_));
